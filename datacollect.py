...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process some lattice data.')
    ...
    output_directory = os.path.join(base_directory, "Collected")
    os.makedirs(output_directory, exist_ok=True)

    L = args.L
    disorder = args.disorder
    disorder_str = f"{disorder:.3f}"

    lattice_path = os.path.join(base_directory, f"L_{L}")
    output_file = os.path.join(output_directory, f'{L}_{disorder_str}.h5')
    print(output_directory, output_file)


    results = {}

    base_directory = os.getcwd()  # Replace this with the root directory
    output_directory = os.path.join(os.getcwd(),"Collected")   # Directory where output files will be stored
    ...
    def merge_h5_for_lattice_and_p(base_dir, lattice_dir, p_dir, output_file):
        ...
        with h5py.File(output_file, 'w') as output_h5:
            ...
            for parity in ['PT']:
                parity_path = os.path.join(p_path, parity)
                if not os.path.isdir(parity_path):
                    continue  # Skip if the PT dir does not exist
                parity_group = output_h5.require_group(parity)
                entries = os.listdir(parity_path)
                Nseeds = sum(1 for entry in entries if os.path.isdir(os.path.join(parity_path, entry)))
    ...
    L_dir = os.path.join(base_directory, "L_%s" % L)
    p_dir = os.path.join(L_dir, "p_%s" %disorder_str)
    merge_h5_for_lattice_and_p(base_directory,L_dir , p_dir, output_file)
    ...
    with h5py.File(output_file, 'r+') as h5_file:
        ...
        # Extract Z_TOT and Z_TOT_unc from 0dis
        ...
        # Store the results back in the same HDF5
        h5_file.create_dataset('Zvec_MC', data=Zratios_final_tot)
        h5_file.create_dataset('Zunc_vec_MC', data=Zratios_final_tot_unc)

        # Store the values from PT as well if it exists
        Zvec_PT = []
        Zunc_vec_PT = []
        if 'PT' in h5_file:
            pt_group = h5_file['PT']
            for seed_name in pt_group:
                if seed_name.startswith('Seed_'):
                    seed_group = pt_group[seed_name]
                    if 'Z_TOT' in seed_group and 'Z_TOT_unc' in seed_group:
                        Z_TOT = seed_group['Z_TOT'][()]
                        Z_TOT_unc = seed_group['Z_TOT_unc'][()]
                        Zvec_PT.append(Z_TOT)
                        Zunc_vec_PT.append(Z_TOT_unc)
        Zvec_PT = np.array(Zvec_PT)
        Zunc_vec_PT = np.array(Zunc_vec_PT)
        h5_file.create_dataset('Zvec_PT', data=Zvec_PT)
        h5_file.create_dataset('Zunc_vec_PT', data=Zunc_vec_PT)
        ...
        for h5_file_name in os.listdir(os.getcwd()):
            if h5_file_name.endswith('.h5') and h5_file_name.startswith(f'L_{lattice_size}_p_'):
                ...
                with h5py.File(h5_file_path, 'r+') as h5_file:
                    ...
                    # Step 2: Multiply the result by Z_TOT
                    Zratios_final_tot, Zratios_final_tot_unc = Zmultiplication(
                        Zratios_final, Z_TOT, Zratios_final_unc, Z_TOT_unc
                    )
